...
export default function AuthScreen({ navigation }) {
    ...
    const handleSendOtp = async () => {
        ...
        try {
            ...
            const vId = await phoneProvider.verifyPhoneNumber(fullPhoneNumber, recaptchaVerifier.current);
            setVerificationId(vId);
            setIsOtpSent(true);
            setLoading(false);
        } catch (error) {
            setLoading(false);
            Alert.alert('Error', error.code === 'auth/billing-not-enabled'
                ? 'OTP service quota exceeded. Please use Password Login.'
    ...
    const handleVerifyOtp = async () => {
        ...
        try {
            ...
            setLoading(false);
            // For OTP login, default to customer flow (can be enhanced with backend user lookup)
                                ...
                                {!isOtpSent ? (
                                    <>
                                        <View style={styles.tabContainer}>
                                            ...
                                            >
                                                <Text style={[styles.tabText, authMode === 'password' && styles.activeTabText]}>LOGIN</Text>
                                            </TouchableOpacity>
                                            <TouchableOpacity
                                                style={[styles.tab, authMode === 'otp' && styles.activeTab]}
                                                onPress={() => setAuthMode('otp')}
                                            >
                                                <Text style={[styles.tabText, authMode === 'otp' && styles.activeTabText]}>OTP</Text>
                                            </TouchableOpacity>
                                        </View>
                                        ...
                                        {authMode === 'password' ? (
                                            <View style={styles.inputSection}>
                                                ...
                                                <TouchableOpacity style={styles.mainBtn} onPress={handlePasswordAction} disabled={loading}>
                                                    {loading ? <ActivityIndicator color={COLORS.white} /> : <Text style={styles.btnText}>{isRegistering ? 'REGISTER' : 'LOGIN'}</Text>}
                                                </TouchableOpacity>
                                    ...
                                    <View style={styles.otpSection}>
                                        ...
                                        <Text style={styles.otpSubtitle}>Enter the 6-digit code sent to you</Text>
...
const styles = StyleSheet.create({
    ...
    otpSubtitle: { fontSize: 14, color: COLORS.grey, marginBottom: SPACING.xl },
